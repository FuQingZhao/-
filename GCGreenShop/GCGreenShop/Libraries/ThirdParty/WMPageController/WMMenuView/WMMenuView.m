...
- (void)calculateItemFrames {
    ...
    for (int i = 0; i < self.titlesCount; i++) {
        ...
        if ([self.delegate respondsToSelector:@selector(menuView:offsetYForItemAtIndex:)]) {
            offsetY = [self.delegate menuView:self offsetYForItemAtIndex:i];
        }
        CGRect frame = CGRectMake(contentWidth, offsetY, itemW, self.frame.size.height);
        // 记录frame
