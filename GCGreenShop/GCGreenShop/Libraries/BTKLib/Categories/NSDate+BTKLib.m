...
@implementation NSDate (BTKLib)

#pragma mark - base date time helpers
...
+ (NSString *)timeInfoWithDateString:(NSString *)dateString {
    ...
    if (time <= 5) {  // 小于5秒
        ...
        return [NSString stringWithFormat:@"%.0f秒前", retTime];
...
#pragma mark - Internet date time helpers
...
- (CGFloat)age {
    ...
    return floorf((CGFloat)daysAgo / 365.0);
}

+ (CGFloat)ageWithDate:(NSDate *)date {
    NSInteger daysAgo = [NSDate daysAgo:date];
    return floorf((CGFloat)daysAgo / 365.0);
}
